...
def create_task_txt(imgs_paths : Union[str, list[str]], all_names : list[str], task_names : list[str], task_name : str, other_criteria = None, return_stats = False) -> str:
    ...
    if return_stats:
        stats_classes = np.zeros(len(all_names), dtype=np.int16)
        has_class_oi = False
    ...
    if is_coco:
        ...
        for img_path in paths:
            ...
            if os.path.isfile(str_label_file):
                ...
                if return_stats:
                    classes_counters = np.zeros(len(all_names), dtype=np.int16)
        ...
        if return_stats:
            stats_classes_task = np.zeros(len(all_names), dtype=np.int16)
